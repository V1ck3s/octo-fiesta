...
/// <summary>
/// Amazon SquidWTF HTTP via curl-impersonate (Chrome TLS). Required for /api/search on edgedragon.
/// </summary>
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                if (process.ExitCode != 0 && !int.TryParse(statusText.Trim(), out _))
                {
                    throw new InvalidOperationException(
                        $"curl-impersonate ({_curlBinary}) failed (exit {process.ExitCode}): {stderr.Trim()}");
                }

                if (!int.TryParse(statusText.Trim(), out var statusCode))
                {
                    throw new InvalidOperationException(
                        $"curl-impersonate returned unexpected status output: '{statusText.Trim()}'");
                }
